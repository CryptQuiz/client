...
const Questions = ({ questions, handleDeleteClick }) => {
  return (
   <div className="flex flex-col w-auto overflow-hidden p-3 space-y-2 bg-[#D2D2D2] text-black rounded-20 border border-black border-r-4 border-b-4">
    ...
    <span className='font-medium'> Toplam soru sayısı:</span> <span className='font-bold'>{questions.length}</span>
    </div>
    </div>
    <div className='w-full h-[20vh] '>
    <QuestionsSwiper questions={questions} handleDeleteClick={handleDeleteClick}/>
    </div>
  </div>
  )
}
